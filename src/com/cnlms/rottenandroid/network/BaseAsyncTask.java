...
public abstract class BaseAsyncTask extends AsyncTask<Void, Void, Void>{
    ...
    /**
     *
     * This method run on the UI thread.
     *
     * It just displays a Progress Dialog indicating that a
     * process is being done in the background.
     *
     */
    @Override
    ...
    /**
     *
     *  Method that makes the background operation,
     *
     *  making a network connection to the remote API and receiving
    ...
    private String generateUrl() throws UnsupportedEncodingException {
        ...
        //sb.append("http://api.rottentomatoes.com/api/public/v1.0/lists/movies/in_theaters.json");
        sb.append("?apikey=" + API_KEY);
        sb.append("&page_limit=20");
        sb.append("&page=");
    ...
    /**
     *
     *  Make the http connection and receives the result
     *
     * @return
     * @throws Exception
     */
    private JSONObject fetchData() throws Exception {
        ...
        URL my_url = new URL("http://www.rottentomatoes.com/");
        HttpURLConnection conn = (HttpURLConnection) my_url.openConnection();
        InputStream is = null;//conn.getInputStream();
        ...
        try {
            ...
            if (status == 200) {
            	...
            	Log.e(LOG_TAG, "Fetching Data Succeeded! ");
    ...
    /**
     *
     *  This method is also run on the UI thread.
     *
     *  It will dismiss the progress dialog and decide the next action
     *
     * @param aVoid
     */
    @Override
